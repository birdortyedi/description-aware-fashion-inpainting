...
class LocalDiscriminator(nn.Module):
    def __init__(self):
        ...
        self.d_block_3 = self._conv_in_lrelu_block(in_channels=48, out_channels=64, kernel_size=3, padding=1)
        self.d_block_4 = self._conv_in_lrelu_block(in_channels=64, out_channels=96, kernel_size=3, stride=2, padding=1)
        self.d_block_5 = self._conv_in_lrelu_block(in_channels=160, out_channels=64, kernel_size=3, padding=1)
        self.d_block_6 = self._conv_in_lrelu_block(in_channels=64, out_channels=16, kernel_size=3, stride=2, padding=1)
        self.avg_pooling = nn.AdaptiveAvgPool2d(output_size=(1, 1))
        self.d_block_9 = nn.Linear(in_features=80, out_features=1)

    def forward(self, x):
        x_1 = self.d_block_1(x)
        print(x_1.size())
        x_2 = self.d_block_2(x_1)
        print(x_2.size())
        x_1 = torch.cat((x_1, x_2), dim=1)
        print(x_1.size())
        x_1 = self.d_block_3(x_1)
        print(x_1.size())
        x_1 = self.d_block_4(x_1)
        print(x_1.size())
        x_1 = torch.cat((x_1, x_2), dim=1)
        print(x_1.size())
        x_1 = self.d_block_5(x_1)
        print(x_1.size())
        x_2 = self.d_block_6(x_1)
        print(x_2.size())
        x_1 = torch.cat((x_1, x_2), dim=1)
        print(x_1.size())
        x = self.avg_pooling(x_1).squeeze()
        print(x.size())
