...
if torch.cuda.device_count() > 1:
    ...
    coarse = nn.DataParallel(coarse).to(device)
    refine = nn.DataParallel(refine).to(device)
    local_d = nn.DataParallel(local_d).to(device)
    global_d = nn.DataParallel(global_d).to(device)
...
def train(epoch, loader, l_fns, optimizers, schedulers):
    coarse.train()
    refine.train()
    for batch_idx, (x_train, x_desc, x_mask, x_local, local_coords, y_train) in tqdm(enumerate(loader), ncols=50,
                                                                                     desc="Training", bar_format="{l_bar}%s{bar}%s{r_bar}" % (Fore.GREEN, Fore.RESET)):
        ...
        if batch_idx % 50 == 0:
            make_verbose(x_train, x_local, y_train, coarse_output, refine_output, refine_local_output, refine_losses,
                         num_step, batch_idx, epoch)
...
def train_refine(num_step, coarse_output, x_mask, y_train, local_coords, l_fns):
    ...
    single_out = ToTensor()(Resize(size=(32, 32))(F.crop(ToPILImage()(im.cpu()), top.item(), left.item(),
                                                         h.item(), w.item())))
...
def train_coarse(num_step, x_train, x_desc, y_train, l_fns):
    ...
    # writer.add_scalar("Loss/on_step_coarse_content_loss", coarse_content.mean().item(), num_step) writer.add_scalar("Loss/on_step_coarse_style_loss", coarse_style.mean().item(), num_step)
...
def make_verbose(x_train, x_local, y_train, coarse_output, refine_output, refine_local_output, refine_losses, num_step, batch_idx, epoch):
    ...
    print("Step:{} ".format(num_step),
          "Epoch:{}".format(epoch),
          "[{}/{} ".format(batch_idx * len(x_train), len(train_loader.dataset)), "({}%)] ".format(int(100 * batch_idx / float(len(train_loader)))), "Loss: {:.6f} ".format(refine_loss.mean().item()),
...
def evaluate(epoch, loader, l_fn):
    ...
    if batch_idx % 50 == 0:
        print("[{}/{} ".format(batch_idx * len(x_val), len(loader.dataset)),
              "({}%)]\t".format(int(100 * batch_idx / float(len(loader)))), "Loss: {:.4f}".format(val_loss.item()),
              "Content: {:.4f} ".format(val_content.item()),
              "Style: {:.9f} ".format(val_style.item()),
              "Structure: {:.4f} ".format(val_struct.item()), "Adversarial: {:.4f} ".format(val_adversarial.item()))
