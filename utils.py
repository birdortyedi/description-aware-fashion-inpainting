...
class HDF5Dataset(data.Dataset):
    ...
    def _build_descriptions(self):
        ...
        descriptions = [list(map(lambda k: k.decode("latin-1").replace(".", " <eos>").split(" "), desc)) for desc in
        descriptions]
    ...
    def get_params(img, scale, ratio, value=0):
        ...
        for attempt in range(10):
            ...
            if h < img_h and w < img_w:
                ...
                j = random.randint(img_w // 5, 4 * img_w // 5 - w) # 0, img_w - w if isinstance(value, numbers.Number):
                v = value
...
class UnNormalize(object):
    ...
    def __call__(self, tensor):
        ...
        for t, m, s in zip(tensor, self.mean, self.std):
            t.mul_(s).add_(m)
            # The normalize code -> t.sub_(m).div_(s) return tensor
